...
export async function generarLlibre(seleccio, bancs) {
  ...
  // Decidir ritme ABANS de usar numCapitols
  ...
  // FIX 1: Primero declara estructures, después beats
  // FIX 2: Tu JSON tiene {beats: [...]}, no claves por nombre
  ...
  for (let numCap = 1; numCap <= numCapitols; numCap++) {
    ...
    escenes.push({ titol: '', text: `<em>${plantilla('tancament', lectures)}</em>` });
