...
const PROMPT_MASTERS = [
  {
    ...
    descripcio: 'Prompt per thriller serial amb 3 víctimes + mentor. Multilocalitat Vic-Manresa-Solsona. Escala paranoia + cliffhangers cada cap.',
    tip: 'Plantilla 4: Thriller Serial 14 Caps | Plantilla 7: Paranoia 3 Actes',
    prompt: `Ets escriptor thriller serial bestseller. Genera SINOPSI 14 capítols paranoia escala + cliffhangers. LOCACIONS 1-3 CIUTATS: Escena 1-4: Vic boscos Sau → Víctima1 i 2 Escena 5-9: Manresa riu Cardener → Perfilació paranoia Escena 10-14: Solsona casa abandonada → Climax símbol complet REGLA COHERÈNCIA VIATGE: Profilador segueix estrella Vic→Manresa C-17 1h. Manresa→Solsona C-26 40min muntanya. VARIABLES JA OMPLERTES: PROFILADOR: David Ferrer, 41 anys traumat, fòbia sang VÍCTIMA 1: Marta Lluís, 24 anys estudiant VÍCTIMA 2: Jordi Coma, 31 anys mecànic VÍCTIMA 3: Anna Roca, 28 anys infermera RITUAL: Ganivet serra 7 incisions SÍMBOL: Estrella 5 puntes gravada pit MENTOR: Pare David Ferrer, desaparegut 10 anys CIUTAT 1: Vic | ZONA: Boscos Sau CIUTAT 2: Manresa | ZONA: Riu Cardener CIUTAT 3: Solsona | ZONA: Casa abandonada perifèria HOOK CAP 1: "Tercer cos aquest mes Vic boscos Sau. Mateix tall 7 incisions, mateix símbol estrella gravat pit." Pregunta: "Qui marca víctimes?" CAP 1-2: INICI Víctima1 Marta Lluís 24 anys boscos Sau Vic. ENLLAÇ Víctima2 Jordi Coma 31 anys. DESENLLAÇ CLIFF: Mateix ritual ganivet serra 7 incisions. "Quan serà víctima3?" CONTINUÏTAT: Punt 1 estrella gravat pit Marta. CAP 3-4: INICI Víctima2 Jordi Coma riu Cardener Manresa. ENLLAÇ David Ferrer 41 anys entra perfilació. DESENLLAÇ CLIFF: Perfil "assí coneix víctima". "Qui coneix totes 3?" CONTINUÏTAT: Punt 2 estrella pit Jordi. CAP 5-7: INICI Perfilació Manresa David analitza patrons. ENLLAÇ Cap 8 gir. DESENLLAÇ CLIFF OPEN LOOP: "Per què símbol estrella 5 puntes?" MISTERI. CONTINUÏTAT: Paranoia David. Fòbia sang augmenta. CAP 8 BEAT 8 PAYBACK: INICI David descobreix ell era objectiu 4. ENLLAÇ Solsona casa abandonada perifèria. DESENLLAÇ CLIFF PAYBACK: Pare David desaparegut 10 anys viu. És mentor culte estrella. "Mentor viu?" GIR MÀXIM. CONTINUÏTAT: Punt 3 estrella Anna Roca. CAP 9-11: INICI Víctima3 Anna Roca 28 anys apareix viva Solsona lligada. ENLLAÇ Casa abandonada paret símbol complet. DESENLLAÇ CLIFF: Símbol estrella paret 5 punts complet. "Mentor és assassí?" CONTINUÏTAT: Casa perifèria Solsona fred. CAP 12-14 PAYBACK TOTAL: INICI Casa abandonada Solsona nit. David enfronta pare mentor. ENLLAÇ Climax. DESENLLAÇ CLIFF RESOLT: Pare mentor és assassí. Volia venjança mare David morta. Frase: "No hi ha segona oportunitat." CONTINUÏTAT: David trenca estrella paret. Paranoia acaba. PATTERN: Esc1 escena crim, Esc2 psico perfil David, Esc3 paranoia fòbia sang. TONO 50% policial 40% psico 10% personal. SORTIDA: 14 caps INICI/ENLLAÇ/DESENLLAÇ CLIFF + escala paranoia + MULTILOCAL VIC→MANRESA→SOLSONA`
  },
  {
    id: 'PM2',
    ...
    prompt: `Ets escriptor romàntica bestseller. Genera SINOPSI 12 capítols slow burn enemies-to-lovers. VARIABLES: {{prota}}, {{love_interest}}, {{ciutat}}, {{conflicte}}`
  },
  {
    id: 'PM3',
    ...
    prompt: `Ets escriptor suspens psicològic. Genera SINOPSI 10 capítols paranoia... VARIABLES: {{prota}}, {{dubte}}, {{ciutat}}`
  },
  {
    id: 'PM4',
    ...
    prompt: `Ets escriptor històric. Genera SINOPSI 15 capítols segle XIX... VARIABLES: {{prota}}, {{any}}, {{ciutat}}, {{esdeveniment_historic}}`
  }
];
...
// ========================================
// 3. PANTALLA GENERAR LLIBRE - 4 PASOS PINÇA: BOTONS = BANCO
// ========================================
function renderPasosGenerar() {
  const genDiv = document.getElementById('pas1-generes');

  // PINÇA: Agafem objecte sencer per id + nombre exacte
  ...
  // PINÇA: Botons amb id + nom exacte del JSON
...
// PINÇA: Ara rep id + nombre per quadrar amb plantilles
window.selectGenere = (id, nombre) => {
  seleccio.genere = id; // PINÇA CRÍTICA: guardem id "policiac", no "Policial"
...
function renderPlantillesFiltre() {
  ...
  // PINÇA: Filtrar per id del genere, no pel nom
...
// PINÇA: Generar amb baseURL per import generarllibre.js
window.generarLlibre = async () => {
  ...
  try {
    // PINÇA: Si mod no carregat, carregar amb baseURL
    if (!mod) {
...
function renderPromptMasters() {
  ...
  grid.innerHTML = PROMPT_MASTERS.map(pm => `
    <div class="card-34" style="border-color:var(--accent);margin-bottom:20px">
      ...
      <button onclick="navigator.clipboard.writeText(\`${pm.prompt.replace(/`/g, '\\`')}\`);alert('✅ Prompt ${pm.id} copiat!')">
        📋 COPIAR PROMPT
      </button>
...
window.llegirCap = num => {
  ...
  if (!cap) return;
  document.getElementById('text-lectura').innerHTML = `<h2>CAPÍTOL ${cap.num}: ${cap.titol}</h2>` + cap.escenes.map(e => `<h4>${e.titol}</h4><p>${e.text}</p>`).join('');
};
...
async function initApp() {
  try {
    ...
    // PINÇA: Carregar també banco_generos.json si no ve al banc
