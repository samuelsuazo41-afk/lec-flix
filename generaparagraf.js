// generaparagraf.js - Motor Paràgraf V9.9 lec-flix policial
// Fix: sense accents en variables + 15 bancs + 500 paraules
...
export async function resetEstructura() {
  ...
  console.log('🔄 Estructura V9.9 resetejada');
}
...
// Generar paràgraf V9.9 - 500 paraules objectiu
export async function generaParagraf(config, bancs, hist, numCap, numEsc, totalCaps) {
  ...
  const escenaris = (bancs.banco_escenarios || []).filter(e =>
    e.ciutat === ciutat &&
    (!subtubActual || e.nom?.toLowerCase().includes(subtubActual.toLowerCase()))
  );
  ...
  // 6. BUCLE 500 PARAULES AMB BANCS
  let intents = 0;
  ...
  while (paraulesComptades < paraulesObjectiu && intents < 80) {
    ...
    // PRIORITAT 1: banco_lectura - frases llargues
    if (bancLecturaUsat.length > 0) {
      ...
      if (text.length > 30 &&!hist.frasesUsades.includes(text.substring(0,40))) {
        parrafo = parrafo + ' + text; // FIX: sense += dins template
    ...
    if (emocions.length > 0) {
      ...
      parrafo = parrafo + ' + nom + ' sentia ' + emo + ' que li cremava per dins mentre caminava per ' + escenari.nom + '.';
      paraulesComptades = contarPalabras(parrafo);
      continue;
    }
    ...
    if (olors.length > 0) {
      ...
      parrafo = parrafo + ' L\'olor de ' + olor2 + ' s\'enfilava per les parets de ' + escenari.nom + ', barrejant-se amb ' + olor + '.';
      paraulesComptades = contarPalabras(parrafo);
      continue;
    }
    ...
    if (sons.length > 0) {
      ...
      parrafo = parrafo + ' El ' + so2 + ' ressonava llunyà entre els carrers de ' + ciutat + ', acompanyant el ' + so + '.';
      paraulesComptades = contarPalabras(parrafo);
      continue;
    }
  }
  ...
  if (capActe === 2 && numEsc % 2 === 0) {
    parrafo = parrafo + ' De sobte va entendre que tot el que creia sobre el cas era una mentida elaborada durant anys.';
  } else if (capActe === 3 && numEsc === config.escenesPerCap) {
    parrafo = parrafo + ' I finalment va comprendre que el viatge havia valgut la pena malgrat el dolor.';
  } else {
    parrafo = parrafo + ' I va saber que ja no hi havia volta enrere.';
  }
...
console.log('✅ Motor Paràgraf V9.9 carregat - 15 bancs + 500 paraules');